  ...
  const AddInstitutionModal = ({ open, onClose, onSuccess }) => {
    ...
    const validate = () => {
      ...
      tempErrors.rfc = Validation.isEmpty(formData.rfc) ? "El RFC es requerido" : "";
      tempErrors.companyName = Validation.isEmpty(formData.companyName) ? "La Razón social de la empresa es requerida" : "";
      tempErrors.giro = Validation.isEmpty(formData.giro) ? "El giro es requerido" : "";
      tempErrors.web = Validation.validateWeb(formData.web) || "";
      tempErrors.telephoneNumber = Validation.validatePhoneNumber(formData.telephoneNumber) || "";
    
      tempErrors.address = {
        street: Validation.isEmpty(formData.address.street) ? "La calle es requerida" : "",
        city: Validation.validateString(formData.address.city) || "",
        state: Validation.validateString(formData.address.state) || "",
        postalCode: Validation.validatePostalCode(formData.address.postalCode) || "",
      ...
      tempErrors.responsible = {
        ...
        education: Validation.validateString(formData.responsible.education) || "",
      };
      ...
      const addressValid = Object.values(tempErrors.address).every((x) => x === "");
      const responsibleValid = Object.values(tempErrors.responsible).every((x) => x === "");
